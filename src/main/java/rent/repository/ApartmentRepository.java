...
public interface ApartmentRepository extends JpaRepository<Apartment, Integer> {
    ...
    @Query(value = "SELECT * from apartment WHERE MATCH (location) AGAINST(:location) LIMIT :start, :end ANd isActive = true", nativeQuery = true)
    List<Apartment> getApartmentsByLocation(@Param("location") String location, @Param("start") int start, @Param("end") int end);

    @Query(value = "SELECT count(*) FROM apartment WHERE MATCH (location) AGAINST(:location) AND isActive = true", nativeQuery = true)
